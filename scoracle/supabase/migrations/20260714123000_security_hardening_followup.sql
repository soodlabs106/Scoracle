...
as $$
  with visible_profiles as (
    select id, username, favorite_club, avatar_url
    from public.profiles
  ...
  scored_predictions as (
    select
      ...
      p.closeness
    from public.predictions p
    join visible_profiles pr on pr.id = p.user_id
    where auth.uid() is not null
      and p.closeness is not null
      and p.closeness <> 'NOT_SCORED'
      and p.match_week <= selected_match_week
...
as $$
  ...
  select
    ...
    ranked.close_count
  from ranked
  join visible_profiles profile on profile.id = ranked.user_id
